...
/* La classe ServerThread est responsable du traitement des tâches envoyées par les clients au serveur principal. Elle s'exécute dans un thread et gère la communication via des sockets, ainsi que la distribution des traitements d'image en local ou via RMI (Remote Method Invocation). */

/* ServerThread est un traitement en arrière-plan qui prend en charge les requêtes des clients.
Il applique certains filtres en local et délègue les tâches lourdes aux workers via RMI.
Le système est conçu pour être scalable, avec une prise en charge automatique des workers distants.
Une file d’attente garantit la gestion ordonnée des demandes et évite la surcharge du serveur principal. */
